...
class HttpServer extends Server
{
    ...
    public function onRequest($request,$response)
    {
        ...
        $file = $this->root.$uri;
        if($uri == '/' or $uri == $this->index or empty(pathinfo($file,PATHINFO_EXTENSION))){
            $this->bootstrap->onRequest($request,$response);
        } elseif($uri != '/' && is_file($file) && pathinfo($file,PATHINFO_EXTENSION) != 'php') {
            // 非php文件, 最好使用nginx来输出
            ...
            //站点目录下的其他PHP文件
            $this->processDynamic($file,$request,$response);
        }
    }
     ...
     * @param SwooleResponse $response
     */
    protected function processDynamic($file,SwooleRequest $request,SwooleResponse $response)
    {
        if (is_file($file))
        {
